<?=form_open('mata_kuliah/update', array('autocomplete' => 'off'))?>
	<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
		...
		<div>
				...
				<select name="f_mahasiswa" required="TRUE" class="form-control">
					...
					<?php if (isset($mahasiswa)): ?>
						<?php foreach ($mahasiswa as $mhs): ?>
							<option value="<?=$mhs->id?>" <?=select_set($mhs->id, $row->id_mahasiswa)?>>[<?=$mhs->id?>] <?=$mhs->nama?></option>
						<?php endforeach ?>
					<?php endif ?>
				</select>
			</div>
			<div class="form-group">
				...
				<input type="text" name="f_nama_mata_kuliah" value="<?=$row->nama_mata_kuliah?>" required="TRUE" class="form-control">
			</div>
